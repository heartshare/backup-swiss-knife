...
sub dump_www{
	my %remotetar_prop = (www_dir=>srvconf("www_dir"),backdir=>srvconf("www_backdir"),ssh_user=>srvconf("ssh_user"),host=>srvconf("host"),www_excludedir=>srvconf("www_excludedir"));
	my $time_start= scalar localtime ();
	my ($return,$report_log) = remote_tar_inc(\%remotetar_prop);
	...
	if ($return == 0) {
		...
		my $current_backups=get_current_backups(srvconf("www_backdir"));
                ...
                $subject="Incremental Backup Report SRV WWW [Success]";
        } else{
                $subject="Incremental Backup Report SRV WWW [Errors]";
        }

        my $report_output=encode("utf8",read_file($report_log));
        my %mail_prop = (mailto=>backconf('mailto'),body=>"Incremental Backup for SRV WWW begin at $time_start and finishes at $time_end.\n\n See the report messages for more details: \n $report_output",subject=>$subject);
